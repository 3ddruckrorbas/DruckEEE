...
const syncToGitHub = async (filePath, content) => {
    const token = process.env.GITHUB_TOKEN;
    const repo = process.env.GITHUB_REPO;
    const branch = process.env.GITHUB_BRANCH || 'main';

    if (!token || !repo) {
        console.warn(`[GitHub Sync] Skipped for ${path.basename(filePath)}: GITHUB_TOKEN or GITHUB_REPO not set.`);
        return;
    }
    ...
    try {
        let sha = null;
        try {
            const getRes = await axios.get(`${apiUrl}?ref=${branch}`, {
                headers: {
        ...
        console.log(`[GitHub Sync] Successfully synced ${fileName} to ${repo}/${branch}`);
    } catch (error) {
        console.error(`[GitHub Sync] Failed to sync ${fileName}:`, error.response ? error.response.data : error.message);
    }
};
...
const writeJsonFile = (filePath, data) => {
    try {
        ...
        syncToGitHub(filePath, data).catch(err => console.error(`[GitHub Sync] Unhandled error:`, err));
    } catch (e) {
